...
func JWTAuthMiddleware() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		// parts[1] 是获取到 tokenString，我们使用之前定义好的解析 JWT 的函数来解析
		...
		// 将当前请求的 userID 信息保存到请求的上下文c 上
		c.Set(controllers.CtxUserIDKey, mc.UserID)

		c.Next() // 后续的处理函数中，可通过 c.Get(CtxUserIDKey) 来获取当前请求用户的信息
	}
}
